...
public:
    [[nodiscard]]
    bool is_released(KeyboardKey key) const {
        return glfwGetKey(m_window_ptr->glfw_ptr(), static_cast<int>(key)) ==
               GLFW_RELEASE;
    }

    [[nodiscard]]
    bool is_pressed(KeyboardKey key) const {
        return glfwGetKey(m_window_ptr->glfw_ptr(), static_cast<int>(key)) ==
               GLFW_PRESS;
    }
