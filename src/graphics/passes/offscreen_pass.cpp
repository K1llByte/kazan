...
void OffscreenPass::render(vk::CommandBuffer& cmd_buffer) {
    ...
    auto view =
        Registry::registry.view<Transform2DComponent, SpriteComponent>();
    view.each([&](auto entity, Transform2DComponent& tc, SpriteComponent& sc) {
        // Draw Commands
        vk::cmd_set_viewport(
        ...
        // Update parameters
        sc.params.mat = tc.matrix();
        sc.render_data.ubo.upload(sc.params);

        m_pipeline.bind(cmd_buffer);
        sc.render_data.dset.bind(cmd_buffer, m_pipeline.layout());
