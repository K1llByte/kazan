...
CommandBuffer::CommandBuffer(Device& device, VkCommandPool pool) {
    ...
    auto result =
        vkAllocateCommandBuffers(device, &alloc_info, &m_vk_cmd_buffer);
...
CommandPool::CommandPool(Device& device)
    ...
    pool_info.queueFamilyIndex =
        device.queue_families().graphics_family.value();

    auto result =
        vkCreateCommandPool(m_device, &pool_info, nullptr, &m_vk_cmd_pool);
