...
RenderPass::RenderPass(Device& device, RenderPassParams&& params)
    : m_device{device}
        ...
        [](const SubpassDesc& sd) {
            return VkSubpassDescription{
                ...
                .inputAttachmentCount =
                    static_cast<uint32_t>(sd.input_attachments.size()),
                .pInputAttachments = sd.input_attachments.data(),
                .colorAttachmentCount =
                    static_cast<uint32_t>(sd.color_attachments.size()),
                ...
                .preserveAttachmentCount =
                    static_cast<uint32_t>(sd.preserve_attachments.size()),
    ...
    render_pass_info.dependencyCount =
        static_cast<uint32_t>(params.dependencies.size());
    render_pass_info.pDependencies = params.dependencies.data();

    auto result = vkCreateRenderPass(
        m_device, &render_pass_info, nullptr, &m_vk_render_pass
    );
...
RenderPass::~RenderPass() {
    vkDestroyRenderPass(m_device, m_vk_render_pass, nullptr);
...
void RenderPass::begin(
    ...
    vkCmdBeginRenderPass(
        cmd_buffer.vk_cmd_buffer(), &begin_info, VK_SUBPASS_CONTENTS_INLINE
    );
}
...
Framebuffer::~Framebuffer() {
    if (m_vk_framebuffer != VK_NULL_HANDLE) {
        vkDestroyFramebuffer(m_device, m_vk_framebuffer, nullptr);
    }
}
